...
namespace StdLib::Allocator
{
    ...
    {
        template <uiw Alignment, typename T = std::byte> [[nodiscard]] UNIQUEPTRRETURN ALLOCATORFUNC RETURNS_NONNULL static T *Allocate(uiw count)
        {
			return MallocAlignedPredefined<Alignment>::template Allocate<T>(count);
        }

        template <uiw Alignment, typename T> [[nodiscard]] ALLOCATORFUNC RETURNS_NONNULL static T *Reallocate(T *memory, uiw count)
        {
