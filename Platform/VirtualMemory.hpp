...
namespace StdLib::VirtualMemory
{
    ...
    [[nodiscard]] Result<PageModes::PageMode> PageMode(const void *memory, uiw size); // Possible errors are InconsistentProtection on Windows, always returns Unsupported on POSIX
    Error<> PageMode(void *memory, uiw size, PageModes::PageMode pageMode); // can commit uncommitted memory
